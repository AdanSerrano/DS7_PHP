...
INSERT INTO noticias VALUES (5, 'Promocion en Costa Mar', 'Con vistas al campo de golf, magnificas calidades, entorno ajardinado con piscina y servi    cio de vigilancia.', 'costas', '2019-02-09', 'apartamento9.jpg');
...
-- CREATE SP LIST NOTICIAS BY CATEGORY
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_listar_noticias_filtro`(
IN param_campo varchar(255),
IN param_valor varchar(255)
)
BEGIN 
    SET @s = CONCAT("SELECT id, titulo, texto, categoria, fecha, imagen FROM noticias WHERE ", 
                    param_campo, " LIKE CONCAT('%", param_valor,"%')");
			PREPARE stmt FROM @s; 
            EXECUTE stmt;
